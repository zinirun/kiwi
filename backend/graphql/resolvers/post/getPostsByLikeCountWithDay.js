...
module.exports = async ({ term }, { departmentId }) => {
    const query = `
                    select p.id, p.title, c.companyName, g.gradeName, u.userName as authorName, p.createdAt, p.updatedAt, cg.categoryName, ifnull(pl.likeCount, 0) as likeCount, ifnull(cm.commentCount, 0) as commentCount
                    from post p
