...
module.exports = async ({ id }, { departmentId, id: userId }) => {
    const query = `
                    select p.id, b.id as boardId, p.authorId, b.boardName, b.link as boardLink, p.title, p.content, g.gradeName, u.userName as authorName, p.createdAt, p.updatedAt, cg.categoryName, ifnull(ppl.likeCount, 0) as likeCount, ifnull(pc.commentCount, 0) as commentCount
                    from post p
